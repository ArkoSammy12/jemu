...
import io.github.arkosammy12.jemu.systems.common.Display;
...
import static io.github.arkosammy12.jemu.systems.gameboy.GameBoyBus.*;
...
public class DMGPPU<E extends GameBoyEmulator> extends Display<E> implements Bus {
    ...
    private void tickSpriteFifo() {
        switch (this.spriteFifoStep) {
            ...
            case 7 -> {
                ...
                for (int i = 0; i < 8; i++) {
                    Integer currentQueuedPixel = i < this.spriteFifo.size() ? this.spriteFifo.get(i) : null;
                    if (currentQueuedPixel == null || getColorNumberFromPixelEntry(currentQueuedPixel) == 0) {
                        ...
                        if (i < this.spriteFifo.size()) {
                            this.spriteFifo.set(i, pixelEntry);
                        } else {
                            this.spriteFifo.offer(pixelEntry);
                        }
                    }
                }

                /*
                for (int c = this.spriteFifo.size(); c < shifted; c++) {
                    int bit = xFlip ? 1 << c : 1 << (7 - c);
                    int low = (this.spriteFifoTileDataLow & bit) != 0 ? 1 : 0;
                    int high = (this.spriteFifoTileDataHigh & bit) != 0 ? 1 : 0;
                    int colorNumber = (low | (high << 1));
                    this.spriteFifo.offer(createPixelEntry(colorNumber, priority, palette));
                }
    ...
    private void tickPixelShifter() {
        if (!this.backgroundFifo.isEmpty()) {
            ...
            if (!this.spriteFifo.isEmpty()) {
                int spritePixel = this.spriteFifo.poll();
