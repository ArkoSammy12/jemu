...
public class DebuggerPanel extends JPanel implements EmulatorInitializerConsumer {
    ...
    public DebuggerPanel(Jemu jemu, MainWindow mainWindow) {
        ...
        jemu.addFrameListener(((systemAdapter, currentState) -> this.onFrame(Optional.ofNullable(systemAdapter).map(SystemAdapter::getEmulator).orElse(null) ,currentState)));
